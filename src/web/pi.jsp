   ...
   <body>
    ...
    <c:if test="${showSubStreams}">
         ...
         <sql:query var="createdStreams">
            select taskname, stream, streamid, Initcap(streamstatus) streamStatus, createDate, StartDate, EndDate
            from stream
               join Task using (Task)
               join (select Stream ParStream, SubTask from ProcessInstance join CreateSubTaskCondition using (Process) where ProcessInstance=?) PS on (PS.ParStream = ParentStream and PS.SubTask = Task)
            where isLatest=1
